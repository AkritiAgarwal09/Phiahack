...
CREATE OR REPLACE FUNCTION public.get_public_mood_boards(search_query text DEFAULT NULL::text, max_items integer DEFAULT 30)
 RETURNS TABLE(id uuid, name text, description text, cover_image text, user_id uuid, creator_name text, creator_avatar text, item_count bigint, view_count bigint, preview_images text[], top_caption text, created_at timestamp with time zone)
 LANGUAGE sql
...
AS $function$
  ...
  LEFT JOIN LATERAL (
    SELECT mi.notes AS note
    FROM public.mood_board_items mi
    WHERE mi.board_id = b.id
      AND mi.notes IS NOT NULL
      AND length(trim(mi.notes)) > 0
